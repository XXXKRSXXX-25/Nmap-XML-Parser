...
def output_results(results, args):
    ...
    # CSV
    if args.csv:
        fields = ["ip", "port", "service", "version"]
        if args.output:
    ...
    for r in results:
        ...
        if args.color and not args.output:
            if args.service:
                ...
                line = f"{GREEN}{r['ip']}{RESET}:{CYAN}{r['port']}{RESET}/{YELLOW}{svc}{RESET}{ver}"
            else:
                line = f"{GREEN}{r['ip']}{RESET}:{CYAN}{r['port']}{RESET}"
...
def main():
    ...
    parser.add_argument("--match", help="Regex match (e.g. apache)")
    ...
    try:
        ...
        # argparse already prints error → just exit cleanly
        sys.exit(1)
